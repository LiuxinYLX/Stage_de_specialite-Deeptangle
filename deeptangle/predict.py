...
def _suppress(x, p, i, cutoff, threshold):
    ...
    # visible est un tableau de boolean
    mask[visible] = np.sum((p[i] - p[visible]) ** 2, -1) >= threshold
    mask[i] = True
    return mask
# i : indice
# cutoff is the distance above which we no longer consider the predictions to be targeting the closest label
...
def non_max_suppression(
    ...
    # sauvegarder les indices pour lesquelles les prédictions sont valides
    remaining_ind = remaining_ind[valid]
    # calculer la position du centre de chaque prédiction dans x
